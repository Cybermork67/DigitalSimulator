...
DFlipFlop::~DFlipFlop() {
    ...
    // Nach dem Speichern Ausgang aktualisieren
    m_output = m_storedState;

    // Falls ihr Memoization mit m_isCalculated benutzt:
    m_isCalculated = true;
}

void DFlipFlop::printState() const {
    std::cout << "[DFlipFlop] " << m_name
              << " | storedState = " << (m_storedState ? "true" : "false")
              << " | output = " << (m_output ? "true" : "false")
              << std::endl;
}